...
+ You should change `[{d1:['1.jpg']}]` to the array of asset-bundles you want to
  track with a progress-bar.
...
+ The [unit test,](http://richplastow.com/w4/support/a4-test.html) which has the
...
);r(P[p.I+1])}}c.add(G?C:L+100);r(P[0])}('',[{d1:['1.jpg']}],[])</script>
    ...
    function r (p) { // `p` will be `undefined` after the last bundle
        ...
        for (k in p)
            if (j = p[k][1]) // ignore `B`, `S`, `O`, `F` and `T`
              ...
              , i.style.cssText = 'width:9em' // good for debug
